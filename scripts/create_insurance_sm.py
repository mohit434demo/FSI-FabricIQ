"""
create_insurance_sm.py — Patch, publish, and refresh the InsuranceSM semantic model.

Expects InsuranceSM.SemanticModel files to already be present locally (e.g. downloaded
...
Usage:
    python scripts/create_insurance_sm.py \
        --workspace-id  <WORKSPACE_GUID> \
        --lakehouse-id  <LAKEHOUSE_GUID> \
        --sm-dir        <path/to/InsuranceSM.SemanticModel>

    # or via environment variables (sm-dir defaults to ./builtin/InsuranceSM.SemanticModel):
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Patch, publish, and refresh InsuranceSM semantic model in Fabric."
    )
    parser.add_argument(
        "--workspace-id",
    ...
    parser.add_argument(
        "--sm-dir",
        default=os.environ.get("FABRIC_SM_DIR", "./builtin/InsuranceSM.SemanticModel"),
        help="Path to the InsuranceSM.SemanticModel folder (default: ./builtin/InsuranceSM.SemanticModel)",
    )
    parser.add_argument(
        "--skip-refresh",
        action="store_true",
...
def main():
    ...
    if not sm_dir.exists():
        ...
        print("Download the InsuranceSM.SemanticModel files first (e.g. run the setup notebook).")
        sys.exit(1)
