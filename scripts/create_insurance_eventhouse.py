"""
create_insurance_eventhouse.py — Create an Eventhouse, KQL Database, and tables in Fabric.

Creates `eh_insurance` (Eventhouse) and `insurance_db` (KQL Database) in the target
...
Usage:
    python scripts/create_insurance_eventhouse.py \\
...
EVENTHOUSE_NAME = "eh_insurance"
KQL_DB_NAME     = "insurance_db"
...
_KQL_DDL_STATEMENTS = [
    ...
    """.alter table ClaimStatusEvent policy retention ```{ "SoftDeletePeriod": "365.00:00:00", "Recoverability": "Enabled" }```""",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Create Eventhouse and KQL Database for the Insurance Claims workshop."
    )
    parser.add_argument(
        "--workspace-id",
